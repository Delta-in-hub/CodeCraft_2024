...
uint32_t barrier_cnt, space_cnt, ocean_cnt;
...
int whichMap() {
  ...
  for (int i = 25; i <= 100; i += 2) {
    int cnt = 0;
    for (int j = 0; j < 200; j++) {
      if (Map::_grids[i][j]._type == Map::Type::barrier) {
        cnt++;
      }
    }
    if (not(cnt > 25 and cnt < 150)) {
      flag = false;
      break;
    }
  }
...
public:
  ...
  uint32_t distanceToRobot(uint32_t rid) const {
    ...
    if (robo.isWithCargo()) {
      ...
      auto man = Map::manhattanDistance({cargo._origin_x, cargo._origin_y},
                                        {robo._x, robo._y});

      auto abss = abs(rd2b - cd2b);
  ...
  float getScore() const {
    ...
    // sc = (std::exp(price / 200)) / (avg_rob + nb_dis);
    // } else {
...
void shipsUpdate() {
  ...
  if (likely(not ships_at_berth.empty())) {
    ...
    for (const auto shid : ships_at_berth) {
      ...
      if (whichmap == 1 or whichmap == 2) {
        free_upbound = std::exp((float)ship._size / (float)Ship::capacity) *
                       getRandom(30, 60);
      } else {
        free_upbound = getRandom(50, 100);
      }
